...
int main()
{
	...
	*/
	...
	ULONG64 ShellcodeAddress = CodeSectionEnd - 0x30; 	
	...
	res = VirtualProtectEx(hProcess, ShellcodeAddress, 8, PAGE_EXECUTE_READWRITE, &res);
	if( res == NULL)
		printf("READ_WRITE_EXECUTE rights failed. \n");
	...
	res = WriteProcessMemory(hProcess, ShellcodeAddress, &shellcode, sizeof(shellcode), &bytes_read);
	if(!res){
		printf("WriteProcessmemory failed writing shellcode: %d \n", GetLastError());
	} else {
